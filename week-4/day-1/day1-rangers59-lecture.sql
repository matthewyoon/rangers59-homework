...
-- Query for a first_name that equals Nick using the LIKE and the WHERE clauses
SELECT first_name, last_name
FROM actor
...
-- Query for all first_names that start with K and have 2 letters after the K using LIKE/WHERE clauses -- combine wildcard and underscore symbols
SELECT first_name, last_name
FROM actor
...
-- Greater OR Equal ( >= ) -- Less OR Equal To( <= )
-- Not Equal ( <> )
...
-- Using the BETWEEN with the AND clauses -- ATTACHED to the WHERE clause
SELECT customer_id, amount
FROM payment
